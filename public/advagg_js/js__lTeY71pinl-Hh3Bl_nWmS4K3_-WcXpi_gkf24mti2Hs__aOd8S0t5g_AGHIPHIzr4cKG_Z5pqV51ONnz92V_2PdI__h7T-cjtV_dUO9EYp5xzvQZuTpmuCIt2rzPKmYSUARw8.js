...
// Altered original hyphenate function to add minWordLength as a parameter.
window['Hypher']['languages'] = {};
}());(function ($) {
    $.fn.oxweb_subsite_hyphenate = function (language, minWordLength) {
        if (window['Hypher']['languages'][language]) {
            return this.each(function () {
                var i = 0, len = this.childNodes.length;
                for (; i < len; i += 1) {
                    if (this.childNodes[i].nodeType === 3) {
                        this.childNodes[i].nodeValue = window['Hypher']['languages'][language].hyphenateText(this.childNodes[i].nodeValue, minWordLength);
                    }
                }
            });
        }
...
(function ($) {
  Drupal.behaviors.subsiteNavigation = {
    attach: function (context, settings) {
      ...
      var $subsite__menu_wrapper = $(".subsite--menu-wrapper", context);

      if ($subsite__menu_wrapper.length > 0) {
        // Loop over menus.
        $subsite__menu_wrapper.each(function () {
          var $wrapper = $(this);

          // Create subsite mobile navigation if there is a subsite menu on the page.
          if ($wrapper.has(".block-menu-block").length > 0) {
            // Add a toggle button (aka trigger) for the menu.
            var trigger_markup =
              '<button class="subsite--menu-trigger" type="button">Menu<span></span><span></span><span></span></button>';
            $wrapper.prepend(trigger_markup);
            var $trigger = $(".subsite--menu-trigger");

            // Add events to the trigger.
            $trigger.on("click", function () {
              // If the menu is already open...
              if ($wrapper.hasClass("subsite--menu-wrapper__active")) {
                Drupal.behaviors.subsiteNavigation.closeSubsiteMenu($wrapper, $trigger);
              } else {
                Drupal.behaviors.subsiteNavigation.openSubsiteMenu($wrapper, $trigger);
              }
            });

            // If the trigger button is visible then we're in mobile view. The nav links should not be tabbable.
            if ($trigger.css('display') !== 'none') {
              $wrapper.find("nav a").attr("tabindex", "-1");
            }

            // Listen for window resize, to remove a class and prevent a flash of animation.
            $(window).resize(function () {
              $wrapper.removeClass("subsite--menu-wrapper__closing");
            });
          }

          // Watch for loss of focus on any element inside the subsite.
          $wrapper.focusout(function(event) {
            // When any children of the menu lose focus we want to close the menu. But not immediately
            // because if focus is moving to another child of the same menu, we want it to stay open. So we
            // will wait a moment before performing this check in case focus has been lost only to go to another
            // element in the same menu.
            setTimeout(function() {
              // Check whether any children of the submenu have focus.
              if ($wrapper.find(':focus').length === 0) {
                // No focus inside the submenu, so close the submenu.
                Drupal.behaviors.subsiteNavigation.closeSubsiteMenu($wrapper, $trigger);
              }
     ...
     * Open a subsite menu.
     *
     * @param $subsite__menu_wrapper {jQuery} jQuery element for the menu wrapper.
     * @param $subsite__menu_trigger {jQuery} jQuery element for the menu trigger.
     */
    openSubsiteMenu: function($subsite__menu_wrapper, $subsite__menu_trigger) {
      // Open it.
      $subsite__menu_wrapper.addClass("subsite--menu-wrapper__active");
      $subsite__menu_trigger.addClass("subsite--menu-trigger__active");
      // Make nav links available for tabbing.
      $subsite__menu_wrapper.find("nav a").attr("tabindex", "0");
    },
    /**
     * Close a subsite menu.
     *
     * @param $subsite__menu_wrapper {jQuery} jQuery element for the menu wrapper.
     * @param $subsite__menu_trigger {jQuery} jQuery element for the menu trigger.
     */
    closeSubsiteMenu: function($subsite__menu_wrapper, $subsite__menu_trigger) {
      $subsite__menu_wrapper
        .addClass("subsite--menu-wrapper__closing")
        .removeClass("subsite--menu-wrapper__active");
      // Set the toggle to closed status.
      $subsite__menu_trigger.removeClass("subsite--menu-trigger__active");

      // Take links out of document for keyboard access to mirror visual hiding if we are in mobile view.
      if ($subsite__menu_trigger.css('display') !== 'none') {
        $subsite__menu_wrapper.find("nav a").attr("tabindex", "-1");
      }
    }
  };

  Drupal.behaviors.subsiteHeader = {
    attach: function (context, settings) {
      var $subsite_header_content = $("header.page-header .row");
      var $subsite_body_content = $("#visible-body");
      var $subsite_header_content_height = null;

      // Override the header height-related settings that are set in CSS,
      // as if JS is running, JS will handle the height.
      $subsite_header_content.css("height", "auto");
      $subsite_body_content.css("margin-top", 0);

      function setHeaderHeight() {
        $subsite_header_content_height = $subsite_header_content.outerHeight();
        $subsite_body_content.css(
          "margin-top",
          $subsite_header_content_height + "px"
        );
      }

      function setMenuHeight() {
        $subsite_body_content.find(".subsite--menu-wrapper nav > ul").css({
          height: "calc(100vh - " + $subsite_header_content_height + "px)",
        });
      }
      ...
      // This holds the timer used to ensure
      // we can run a function after a resize.
      // It simulates a debounce.
      var resizeTimer;

      $(window).resize(function () {
        // Run the function after the resize event has stopped
  ...
  Drupal.behaviors.runHypher = {
    attach: function (context, settings) {
      // Run our version of the hypher.js plugin to hyphenate some text.
      $(
        ".field-name-field-title .field-item-single, .field-name-field-hero-slogan .field-item-single"
      ).oxweb_subsite_hyphenate("en-gb", 8);
    },
  };

  /**
   * Autoplay the first Video Driver Slice
   * YouTube video on a page.
   */
  Drupal.behaviors.autoplayYoutubeVideo = {
    attach: function(context, settings) {
      $('body', context).once('autoplayYoutubeVideo', function() {
        // Targets only the first matching Slice.
        var slice = $('.js-autoplay-first-video')[0];
        ...
        if (slice) {
          ...
          // Appends the YouTube Iframe API script to the page.
          scriptTag.src = 'https://www.youtube.com/iframe_api';
          $body.append(scriptTag);

          // The required callback function onYouTubeIframeAPIReady.
          window.onYouTubeIframeAPIReady = function() {
            player = new YT.Player($('.media-youtube-player', slice)[0], {
              events: {
          ...
          /**
           * Immediately mute and play the video.
           * @param {Object} event - The YouTube event dispatched by the 'onReady' event.
           */
